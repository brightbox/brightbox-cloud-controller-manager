...
def get_refs():
    ...
    for path in glob.glob(os.path.join(args.boilerplate_dir,
                                       "boilerplate.*.txt")):
...
def file_passes(filename, refs, regexs):
    ...
    if ref != data:
        print("Header in %s does not match reference, diff:" % filename,
              file=verbose_out)
        if args.verbose:
            print(file=verbose_out)
            for line in difflib.unified_diff(ref, data, 'reference', filename,
                                             lineterm=''):
...
skipped_dirs = [
    ...
    # Imported from Oracle maintaining original header
    "hack/test.sh",
    "hack/verify-boilerplate.sh"
...
def get_regexs():
    regexs = {}
    # Search for "YEAR" which exists in the boilerplate, but shouldn't
    # in the real thing
