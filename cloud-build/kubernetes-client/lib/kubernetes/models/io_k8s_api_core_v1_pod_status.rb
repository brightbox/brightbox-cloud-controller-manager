  ...
  class IoK8sApiCoreV1PodStatus
    ...
    def self.openapi_types
      {
        ...
        :'phase' => :'String',
        :'pod_ip' => :'String',
        :'pod_i_ps' => :'Array<IoK8sApiCoreV1PodIP>',
        :'qos_class' => :'String',
        :'reason' => :'String',
        :'start_time' => :'DateTime'
      }
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Kubernetes::IoK8sApiCoreV1PodStatus` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Kubernetes::IoK8sApiCoreV1PodStatus`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'pod_i_ps')
        if (value = attributes[:'pod_i_ps']).is_a?(Array)
          self.pod_i_ps = value
        end
      end
    ...
    def hash
      [conditions, container_statuses, ephemeral_container_statuses, host_ip, init_container_statuses, message, nominated_node_name, phase, pod_ip, pod_i_ps, qos_class, reason, start_time].hash
    end
