  ...
  class IoK8sApimachineryPkgApisMetaV1APIVersions
    ...
    def self.openapi_types
      {
        :'api_version' => :'String',
        :'kind' => :'String',
        :'server_address_by_client_cid_rs' => :'Array<IoK8sApimachineryPkgApisMetaV1ServerAddressByClientCIDR>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Kubernetes::IoK8sApimachineryPkgApisMetaV1APIVersions` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Kubernetes::IoK8sApimachineryPkgApisMetaV1APIVersions`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
